---
title: "HIC: Gut Microbiota - Patient/Control Analysis (manuscript)"
author: "Alex Gould"
date: "`r date()`"
output:
  pdf_document:
...
otu_or_asv       = "ASV"
read_type        = "filter"
rep_shapes       = rep(c(15, 16, 17, 18, 3, 7, 8, 9, 10, 11, 12, 13), 3)
```
...
Observed, Chao1, and Simpson violated Levene's test of Homogeneity of Variances so a Welch's t-test is used. 
...
## plot figure
set.seed(123)
fbratio_group_plot <- fbratio_group %>% 
...
compare_abundance_comparison <- function (taxa,
  ...
  plot_dat_format <- plot_dat_SE %>%
    ...
    dplyr::mutate(
      ...
      average_Patient = sprintf("%.1f", average_Patient),
      variability_Patient = sprintf("%.1f", variability_Patient),
      Control = paste0(average_Control, " (", variability_Control, ")"),
      Patient = paste0(average_Patient, " (", variability_Patient, ")")
    ) %>%
    dplyr::rename(pval = pval_Patient,
                  padj = padj_Patient,
                  Taxa = TaxName) %>%
    dplyr::mutate(pval = sprintf("%.3f", as.numeric(pval)),
                  padj = sprintf("%.3f", as.numeric(padj))) %>%
    dplyr::select(Taxa, Control, Patient, pval, padj)
...
### Abundance Comparison Plot Function: Normal Text
...
plot_abundance_comparison <- function (taxa,
  ...
  p2 <- plot_dat %>%
    ...
    ggplot(aes(
      ...
      ## flip group order
      fill = factor(Group, levels = c("Patient", "Control"))
    )) +
    geom_boxplot(
      width = 0.6,
  ...
  p3 <- plot_dat %>%
    ...
    geom_jitter(
      ...
      aes(colour = factor(Group, levels = c(
        "Patient", "Control"
      ))),
...
### Abundance Comparison Plot Function: Italicised Text
...
phylum_abund_group <- plot_abundance_comparison("Phylum", nclass = 7)
...
``` {r Section4_DESeq2_analysis_patient_vs_control}
DE_PvC <- diff_out("Patient", "Control", plot = FALSE)
cat("Total number of differentially abundant OTUs: ", nrow(DE_PvC[["Signif"]]))
cat("Upregulated number of differentially abundant OTUs: ", nrow(DE_PvC[["Up"]]))
cat("Downregulated number of differentially abundant OTUs: ", nrow(DE_PvC[["Down"]]))
```

Below is a volcano plot showing the overall differential abundance results for this comparison.

``` {r Section4_DESeq2_analysis_patient_vs_control_volcano, fig.height = 5, fig.width = 7}
volcano_p <- plot_volcano(DE_PvC)
volcano_p
```
...
## Make sure names are still "Control" and "Patient"
...
ggsave(
  filename = paste0(
    analysis_dir,
    "figs/HIC_microbiome_patient_control_panel",
    ".png"
  ),
...
sinfo <- sessionInfo()
sinfo
save.image(paste0(analysis_dir, "/HIC_16S_analysis_manuscript_patient_control.Rout"))
```
