...
## Load data sets
data <- read.xlsx(paste0(data_dir,"HIC ELISA workbook_HTA_PVadjust.xlsx"),
                  sheetIndex = 7,
                  password = rstudioapi::askForPassword(pw_message)) %>%
  ## Recode grouping columns
  dplyr::mutate(Class = dplyr::recode(Class, "1" = "Patient", "2" = "Control")) %>%
  ## Rename Class to Group
  dplyr::rename(Group = Class) %>%
  dplyr::mutate(Outcome = dplyr::recode(Outcome, "1" = "Tolerant", "2" = "Intolerant")) %>%
  dplyr::mutate(Pre_EHI = dplyr::recode(Pre_EHI, "1" = "Yes", "2" = "No")) %>%
  ## Create factor levels of interest
  dplyr::mutate(Group = factor(Group, levels = c("Control", "Patient")),
                Outcome = factor(Outcome, levels = c("Tolerant", "Intolerant")),
                Pre_EHI = factor(Pre_EHI, levels = c("No", "Yes"))) %>%
  ## Remove Delta scores
  dplyr::select(!contains("Delta")) %>%
  ## Change all occurrences of <1 to 0 and set to numeric
  dplyr::mutate(CRP_Pre = if_else(CRP_Pre == "<1", "0", CRP_Pre),
                CRP_Pre = as.numeric(CRP_Pre))
